 ...
 */
function makePaidFetch(runtime) {
...
async function x402Get(runtime, path) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`x402 request failed (${res.status}): ${body.slice(0, 200)}`);
    }
    return res.json();
}
...
const investIdleUsdcAction = {
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        try {
            ...
            const data = (await x402Get(runtime, `/api/x402/invest?usdc=${usdcAmount}&wallet=${account.address}`));
